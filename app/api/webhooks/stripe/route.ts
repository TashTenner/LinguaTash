    ...
    try {
      const items = languages.map((code) => {
        ...
        return {
          ...
          tax_rate: ivaRate,
          aeat_code: '01',
          operation_qualification: (ivaRate === 0 ? 'S2' : 'S1') as 'S1' | 'S2',
        }
      })
      const result = await registerVerifactuInvoice({
        series: 'RESUENA',
        invoiceNumber,
        issueDate,
        invoiceType: nif ? 'F1' : 'F2',
        ...
        customerName: nombreEmpresa || nombre,
        customerNif: nif || null,
        items,
      })
      verifactuQrUrl = result.qrUrl
